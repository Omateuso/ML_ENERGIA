...
def processar_dados(caminho_pasta = 'dataset/2024/INMET_2024'):
    ...
    for arquivo in arquivos:
        try:
            with open(arquivo, 'r', encoding='latin-1') as f:
                ...
                regiao = primeira_linha.split(';')[1].strip()
            # Lendo o arquivo CSV
            ...
            if coluna_temp in df_temp.columns and coluna_umi in df_temp.columns:
                ...
                clima_diario_estacao['id_subsistema'] = regiao
    ...
    # Agrupando por data e região para obter uma média única por subsistema
...
def carregar_e_preparar(caminho_energia, caminho_inmet=None):
    ...
    # Carga do dia anterior, média móvel de 7 dias (captura de tendeência semanal) e organizando por região e dia cronológico
    ...
    # Verificando se o caminho do clima existe e processando os dados de clima
    if os.path.exists(clima_alvo):
        # Processando os dados de clima e integrando com o dataset de energia
        df_clima = processar_dados(clima_alvo)
        # Verificando se os dados de clima foram processados com sucesso
        if df_clima is not None:
            # Realizando o merge entre os dados de energia e clima com base na data e região
...
def treinar_modelo_com_rastreamento_carbono(df, subsistema='SE'):
    # Filtrando dados para o subsistema específico
    ...
    if df_sub.empty:
        ...
        return None
    # Definindo as colunas que o modelo vai usar para prever
    features = ['mes','dia_semana','trimestre','carga_ontem','is_feriado','is_fds','media_7d']
    # Verificando se a coluna de temperatura média está presente no DataFrame e adicionando às features
    ...
    # Iniciando o rastreamento de emissões de carbono
...
if __name__ == "__main__":
    ...
    try:
        ...
        subsistemas = df_preparado['id_subsistema'].unique()
        print(f"Subsistemas encontrados: {subsistemas}")
        for subs in subsistemas:
            print(f"\nTreinando modelo para o subsistema: {subs}")
            modelo_regiao = treinar_modelo_com_rastreamento_carbono(df_preparado, subsistema = subs)
            if modelo_regiao is not None:
                nome_arquivo = f'modelos/modelo_energia_{subs}.joblib'
                joblib.dump(modelo_regiao, nome_arquivo)
                print(f"Modelo para os subsistemas {subs} salvo como {nome_arquivo}")
